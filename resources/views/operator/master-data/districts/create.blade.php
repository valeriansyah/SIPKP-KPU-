...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-text">Tambah Kabupaten/Kota</h1>
        <p class="text-text-secondary mt-1">Tambahkan data wilayah operasional pelaporan yang baru.</p>
    </div>
...
<x-ui.card class="bg-white border border-gray-100 shadow-sm max-w-2xl">
    <form action="{{ route('operator.master-data.districts.store') }}" method="POST" class="p-6">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                @error('code')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-text-secondary">Opsional. Digunakan untuk keperluan integrasi atau pelaporan khusus.</p>
            </div>
        </div>
        ...
        <div class="mt-8 flex justify-end gap-3">
            ...
            <x-ui.button type="submit" variant="primary" class="h-10 px-6">Simpan Data</x-ui.button>
        </div>
    </form>
</x-ui.card>
@endsection
