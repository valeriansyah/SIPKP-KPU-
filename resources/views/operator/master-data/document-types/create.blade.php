...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-text">Tambah Jenis Dokumen</h1>
        <p class="text-text-secondary mt-1">Tambahkan persyaratan lampiran baru untuk pelaporan.</p>
    </div>
...
<x-ui.card class="bg-white border border-gray-100 shadow-sm max-w-2xl">
    <form action="{{ route('operator.master-data.document-types.store') }}" method="POST" class="p-6">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    ...
                    placeholder="Contoh: Fotokopi KTP pelapor yang masih berlaku.">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
